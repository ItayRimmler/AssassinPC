...
while my_game_works or in_credits or in_menu or in_inst or in_scene:
    ...
    if my_game_works:
        ...
        if surf["DBackground"]['rect_Assassinate'].collidepoint(g.pg.mouse.get_pos())  and now - last_c1 >= g.c.SWAP and (symbol == 0 or symbol == 1 or symbol == 2 or symbol == 3) and mouse_pressed and not hearing:
            ...
            mouse_pressed = False
            continue
        if scroll_up and now - last_c2 >= g.c.SWAP and (symbol == 0 or symbol == 1 or symbol == 2 or symbol == 3):
            cards.empty()
            num += 1
            if num == 12:
                num = 0
            cards.add(card_list[symbol][num])
            scroll_up = False
            last_c2 = now + g.c.SWAP
        if scroll_down and now - last_c2 >= g.c.SWAP and (symbol == 0 or symbol == 1 or symbol == 2 or symbol == 3):
            cards.empty()
            num -= 1
            if num == -1:
                num = 11
